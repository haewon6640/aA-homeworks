...
describe Dessert do
  ...
  let(:dessert) { Dessert.new("pastry",1,chef) }
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect{Dessert.new("pastry","1", "Chef Jerry")}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["flour","baking soda","egg","milk"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Jerry the Great Baker")
      expect(dessert.serve).to eq("Chef Jerry the Great Baker has made 1 pastries!")
    end
  end
